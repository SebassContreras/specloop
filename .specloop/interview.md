...
| worker-cli | covered | 3 workers configured: `claude` (`-p`), `codex` (`exec`), `opencode` (`run`) — flags confirmed via web search, not guessed. Whichever harness runs `specloop:loop` uses its own native sub-agent for the matching entry; the other two fall back to CLI subprocess with these args. Written to `.specloop/loop.config.json`. |
