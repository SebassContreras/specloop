...
| worker-cli | covered | 5 workers configured: `claude` (`-p`), `codex` (`exec`), `opencode` (`run`), `copilot` (`--allow-all-tools -p`), `cursor-agent` (`--trust --force -p`) — the first three confirmed via web search, the last two from `022`'s 2026-09-19 audit runs, none guessed. `agy` is deliberately left out: its entry needs this checkout's absolute path, and its headless mode can't run shell commands (see `skills/loop-setup`). Whichever harness runs `specloop:loop` uses its own native sub-agent for the matching entry; the others fall back to CLI subprocess with these args. Written to `.specloop/loop.config.json`; the last two entries added by hand 2026-09-19. |
