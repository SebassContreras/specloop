...
| T2 | Add `existingContextFiles(config, cwd)` — filter `contextFiles` to those on disk | agent | done | Filtered at prompt-build time, not config load: a project can gain `docs/styles.md` between runs. |
...
A fixture repo (`AGENTS.md`, `docs/{architecture,styles,roadmap}.md`, two specs) plus a
...
| Prompt names existing context files | `AGENTS.md, docs/architecture.md, docs/styles.md` |
| Configured-but-absent context file omitted | `docs/nonexistent.md` correctly not mentioned |
