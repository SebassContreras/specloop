...
palette/colour/typography/theme/brand/design-system anywhere in `docs/`, `skills/`,
...
1. **Capture** — question-bank Phase D, gated on `visual-surface`: `palette`,
   ...
   from with reason.
2. **Storage** — `docs/styles.md` for detail (hex values, type stacks, tokens);
...
- `docs/styles.md` is owned by `skills/start`, not a project deliverable the roadmap
  decides. It is loop context, in the same category as `AGENTS.md` — that distinction
  is recorded in `docs/product.md`'s "Out of scope".
- Preference strength must survive to the worker: a hard rule and a soft default must
  not read identically in `AGENTS.md`.

## Acceptance criteria

- A project declaring a visual surface ends with a `docs/styles.md` containing only
  values the user actually chose, and an `AGENTS.md` "Style" section summarising them
  with their strength.
- A project with no visual surface gets no `docs/styles.md` and is never asked about
...
- With `014` in place, a worker prompt on a styled project names `docs/styles.md`.
