...
| T3 | Implement Phase 0: spec resolution/listing from `docs/roadmap.md`, refusal check on empty/stub `requirements.md` | agent | done | Implemented as skill instructions in `SKILL.md` — a Skill's "implementation" is its instructions, not a script. |
...
| T11 | Append settled stack/convention decisions to `docs/architecture.md` and `AGENTS.md` on close | agent | done | This is the writer `skills/start` already promised existed and nothing delivered — `architecture.md` was otherwise a permanent TBD stub. |
