...
- New skill, `specloop:status`, **read-only** — never writes to any file, no
  exceptions.
- Reads the target repo's `docs/roadmap.md` plus every listed spec's `tasks.md` and
...
- Must work standalone without the loop orchestrator installed — `.specloop/` may
  ...
  reads of `docs/`, same parsing contract the orchestrator's `roadmap.ts`/`tasks.ts`
  already use.
