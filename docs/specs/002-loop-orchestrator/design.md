...
- **`loop run`** — start the orchestrator (master terminal). Reads
  `.specloop/loop.config.json`, reads `docs/roadmap.md`, picks the next eligible spec,
...
- Parse `docs/roadmap.md`'s table (fixed format already established by 001).
...
1. Refuse if no spec in `docs/roadmap.md` has a populated `tasks.md` yet (nothing to
...
└── src/
    ...
    ├── roadmap.ts          # parse docs/roadmap.md, pick next eligible spec
