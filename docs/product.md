...
branches everything downstream (see `docs/architecture.md`'s fixed rules).

**One trigger** — "I need to set up X" — does the initial scaffold:

1. Scaffolds a fixed documentation structure in the target repo:
   `CLAUDE.md` + `AGENTS.md` + `docs/product.md` + `docs/architecture.md` +
   `docs/roadmap.md` + `docs/specs/NNN-name/{requirements,design,tasks}.md`, plus
   `docs/styles.md` when the project has a visual surface, plus the `.specloop/`
...
3. Asks about technologies, architecture, and tools — branched by project type, with
   the answers written into `docs/architecture.md` as a decision register. Its section
...
skip is recorded as a skip — never silently dropped. See `docs/architecture.md`.
...
- Does not scaffold `README.md`, `CONTRIBUTING.md`, `LICENSE`, or CI config into the
  ...
  (`CLAUDE.md`, `AGENTS.md`, `docs/styles.md` and `.specloop/` are *not* in this
