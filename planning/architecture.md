...
| `.claude-plugin/marketplace.json` listing | Not needed for a plugin installed via `--plugin-dir` or a direct repo checkout; revisit only if distributing through a plugin marketplace becomes a goal (`005-open-source-release/requirements.md`). Superseded by `036-lean-distribution` on 2026-09-21 (user go-ahead), building on the verified marketplace formats from `035`. |
...
| A proactive master-usage-limit detection/announcement in `skills/loop` (`025-master-handoff`) | 2026-09-17. Declined at design-closing, never built. Researched rather than assumed: no harness (Claude Code confirmed via `anthropics/claude-code` issue #26340; others already suspected) exposes a self-usage-limit signal to the model mid-conversation, so a "notice signs of running low" branch would never fire — documenting one would assert a capability that doesn't exist. The actual gap (a fresh session already resumes an exhausted master's work via `skills/loop`'s existing Phase 1/2 rules and on-disk state alone) needed no new mechanism; the user also declined even a proactive one-line note in `skills/loop/SKILL.md` naming that resume path, as more plugin surface than the already-working behavior warrants. |
| Git-worktree isolation for spec-level sub-agents (`029-spec-worktree-isolation`) | 2026-09-17. Declined at design-closing, never built — its own requirements.md had a self-imposed gate blocking design until `planning/roadmap.md`'s cross-worktree write-reconciliation question had a real answer. That answer (serialize through the master only, already true today: sub-agents never write `roadmap.md`, only the master does, one merge at a time) resolved the blocker, but a separate check made the whole spec moot first: `skills/loop`'s existing file/section-overlap batching already runs genuinely independent tasks concurrently in the one shared working tree, with no git-level collision possible — confirmed against a real, external project run (three concurrent agents, no issue). Worktree isolation would only have bought parallelism for tasks that share a file, which `026`'s own run already suggested is rarer than it looks. |
