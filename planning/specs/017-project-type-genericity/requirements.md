...
anything else that needs a roadmap (`planning/product.md`).
...
- `planning/architecture.md` was scaffolded with `Container`/stack headers and, for a
...
- Type-keyed section headers for `planning/architecture.md` (software → container/stack/
...
`planning/product.md`. Also the `B-other` branch's real purpose: an unanticipated project
...
- The project type is established by the **first** interview question and persisted in
  `planning/product.md`; every downstream branch reads it from disk rather than re-inferring.
...
- `planning/architecture.md`'s scaffolded headers differ by project type.
- `design-closing` and `task-breakdown` run against a project whose
  `planning/architecture.md` is absent.
