...
└── planning/                    # already exists — doubles as the reference
...
stay "still to define" per `planning/architecture.md`.
...
(`AGENTS.md`/`CLAUDE.md`), the preference record (`planning/styles.md`), the `.specloop/`
...
deliverables and declined all of them, citing a `planning/product.md` clause written in the
...
T15–T27 and `planning/architecture.md`'s "Declined" preamble.
...
ledger does it fall back to content detection (`planning/product.md`'s "What this is" real
...
writes `planning/specs/.gitkeep` (the directory can't be committed empty otherwise) and
...
project. Written to both `planning/product.md` and `AGENTS.md` so downstream skills read it
...
`planning/architecture.md` as a decision register (what / choice / why) and the operative
...
it, a first run asked **zero** technology questions, `planning/architecture.md` stayed a
...
whether anything is rendered. Detail to `planning/styles.md`, operative summary plus
