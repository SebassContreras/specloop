...
- [ ] T021 [agent] [status:todo] Recover a task stuck `in_progress` from an ungraceful external kill (not `safeStop`'s clean path) — `nextRunnableTask` currently only resumes `todo`/`interrupted`, so such a task blocks its spec forever without a manual reset
      └─ Found during `006` T010's live run: the invoking shell's own command timeout (not the orchestrator's 30-min `WORKER_TIMEOUT_MS`) killed a `claude -p` call mid-task, leaving it `in_progress`; recovered by hand. Needs a design decision — e.g. treat a stale `in_progress` task (older than some threshold, or unconditionally on the next `loop run`) the same as `interrupted` — before implementing.
