...
- [ ] T005 [human] [status:todo] Run `vhs demo-loop.tape` inside that throwaway repo (after `specloop:loop-setup` + a populated `tasks.md`) to produce `demo-loop.gif`/`demo-loop-status.png`/`demo-loop-run.png`
      └─ The `vhs` command itself is agent-runnable, but the throwaway repo it depends on only exists after T4's live session. `demo-loop.tape` only invokes `loop status`/`loop run` (specloop's own orchestrator binary), so it's unaffected by which harness ran the interview.
