...
- [ ] T004 [human] [status:in_progress] Capture `demo-interview.gif`/`demo-interview.png` from a real `/specloop:start` session in a throwaway repo (ScreenToGif)
      ...
      └─ **2026-09-20**: the Claude Code take shipped instead (`.github/assets/demo-interview-claude.gif` plus three `claude-*.png` captures, `README.md`'s `## Demo` caption says Claude Code again). Codex CLI and OpenCode takes exist but are not yet published; the task stays open until they are.
