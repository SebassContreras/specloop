...
- Reads the target repo's `planning/roadmap.md` plus every listed spec's `tasks.md` and
  ...
  task-breakdown → loop-setup → loop).
- **Reads `roadmap.md`'s `Stage` column directly for this rather than re-deriving
  ...
  than trusting it blindly — this is the read-only disagreement check nothing else
  in the repo does today; `skills/loop` only ever overwrites `Status` from its own
  rollup, it doesn't separately detect or report a stale cell.
- Must work standalone without `.specloop/loop.config.json` written yet — a repo
  that hasn't reached `loop-setup` should still get a useful report. Pure
  Markdown-table reads of `planning/`, same parsing contract `skills/loop` uses.
