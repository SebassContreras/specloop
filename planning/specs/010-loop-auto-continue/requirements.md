...
- `loop run` currently stops once the active spec has no remaining runnable tasks —
  ...
  without constant supervision" — `planning/product.md`).
- `loop run` should re-check `planning/roadmap.md` for the next eligible spec (status
...
- True multi-spec parallelism (running two independent specs' tasks concurrently) —
  already flagged as "still to define" in `planning/architecture.md`; this spec is
